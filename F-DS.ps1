        ...
        ForEach ($TargetPath in $Path) {
            if (($TargetPath -Match '\\\\.*\\.*') -and ($PSBoundParameters['Credential'])) {
                ...
                if (-not $MappedComputers[$HostComputer]) {
                    ...
                    A-RC -ComputerName $HostComputer -Credential $Credential
    ...
    PROCESS {
        ForEach ($Computer in $ComputerName) {
            try {
                ...
                Write-Verbose "[R-IPA] Could not resolve $Computer to an IP Address."
            }
        }
    }
}
...
function G-PC {
    ...
    try {
        if ($PSBoundParameters['Domain'] -or ($Identity -match '.+\\.+')) {
            if ($Identity -match '.+\\.+') {
                # DOMAIN\groupname
                $ConvertedIdentity = $Identity | C-ADN -OutputType Canonical
                if ($ConvertedIdentity) {
                    $ConnectTarget = $ConvertedIdentity.SubString(0, $ConvertedIdentity.IndexOf('/'))
                    $ObjectIdentity = $Identity.Split('\')[1]
                    Write-Verbose "[G-PC] Binding to domain '$ConnectTarget'"
                }
            }
            else {
                $ObjectIdentity = $Identity
                Write-Verbose "[G-PC] Binding to domain '$Domain'"
                $ConnectTarget = $Domain
            }

            if ($PSBoundParameters['Credential']) {
                Write-Verbose '[G-PC] Using alternate credentials'
        ...
        Write-Warning "[G-PC] Error creating binding for object ('$Identity') context : $_"
    }
}
...
function A-RC {
    ...
    PROCESS {
        ...
        ForEach ($TargetPath in $Paths) {
            ...
            Write-Verbose "[A-RC] Attempting to mount: $TargetPath"
            ...
            if ($Result -eq 0) {
                ...
                Throw "[A-RC] error mounting $TargetPath : $(([ComponentModel.Win32Exception]$Result).Message)"
            }
        }
    }
}
...
function R-RC {
    ...
    PROCESS {
        ...
        ForEach ($TargetPath in $Paths) {
            Write-Verbose "[R-RC] Attempting to unmount: $TargetPath"
            ...
            if ($Result -eq 0) {
                ...
                Throw "[R-RC] error unmounting $TargetPath : $(([ComponentModel.Win32Exception]$Result).Message)"
            }
        }
    }
}
    ...
    PROCESS {
        ...
        ForEach ($Object in $TargetObject) {
            ...
            try {
                ...
                Write-Warning "[G-DSPNT] Error requesting ticket for SPN '$UserSPN' from user '$DistinguishedName' : $_"
            }
            if ($Ticket) {
...
function I-K {
    ...
    PROCESS {
        ...
        Get-DomainUser @UserSearcherArguments | Where-Object {$_.samaccountname -ne 'krbtgt'} | G-DSPNT -OutputFormat $OutputFormat
    }
